...
- `papers.yml` holds structured data on every paper. **The source of truth for every number.**
- `operators.yml` holds the 26 LLM forward-pass operators and how each scheme proves each one.
- `references/<cell>/*.pdf` are the PDFs we have actually read. Cells: `proving-inference`,
...
- `content/<section>/*.md` is the prose. **This is what you write.**
- `content/papers/<id>.md` is an optional deep-dive appended to an auto-generated paper page.
- `site/build.py` joins all of the above into `docs/`.
- `site/validate.py` fails the build when data and prose drift apart.
- `site/sections.yml` declares sections and maps each `papers.yml` category to a section.
- `docs/` is **GENERATED and GITIGNORED. Never hand-edit.** CI rebuilds and publishes it.
- `README.md` is a summary of the site. It should shrink, not grow; new prose belongs on the
  website, not here.
...
- **Numbers live in `papers.yml`, never in prose.** To say "DeepProve hits 174 tok/min" you do
  ...
  `x faster`, and similar figures and fails you. The one exception is a figure inside quotation
...
Then wire up the website surface so the paper is never orphaned: the owning `papers.yml` category
must be claimed by a section in `site/sections.yml`, and a `content/<section>/*.md` page should
...
prove it. Do not narrate, and do not hedge with filler ("it is worth noting", "interestingly",
...
- `:::gap` marks a boundary of knowledge: nobody has done X, no paper reports Y, a claim is
  scoped narrower than it sounds.
- `:::debate` marks two sources or two readings that genuinely disagree, and that this SoK
  deliberately does not resolve.
- `:::audit` marks what to distrust: where a bug hides, a security surface, an unverified vendor
  claim, a soundness question.
- `:::intuition` marks non-adversarial teaching: the mental model, why a mechanism works.
- `:::quote{src="…" sec="…"}` is **verbatim** from a PDF in `references/`. If you cannot
  copy-paste it out of `pdftotext`, paraphrase in prose instead; do not use `:::quote`.

Use `[[wikilinks]]` to paper ids on first mention, and shortcodes (`{{ table:… }}`,
`{{ perf:… }}`, `{{ chart:… }}`, `{{ papers:… }}`, `{{ coverage }}`, `{{ landscape }}`) instead
of copied figures.

## 4. Verify before committing

- `make site` rebuilds `docs/`. It must succeed.
- `make check` is the integrity gate. It must pass. It enforces: every `papers.yml` category is
